...
def game(test_mode = False, number_hobos = 1):
    ...
    if test_mode:
        NUMBER_HOBOS = number_hobos
    if not test_mode and type(hobo_num) == int:
    ...
    if not test_mode:
        # Set up splash screen if not test mode
    ...
    class Hobo(pygame.sprite.Sprite):
        ...
        def switchTrack(self):
            ...
            # Only switch if the hobo is not dead
    ...
    if test_mode:
        ...
        # Show Splash screen, only if not in test mode
    ...
    # To quit normally on a ctrl-c in terminal or x if windowed, not to run during test mode
    if not quit_induced and not test_mode:
        render()
        # Play game over sfx
